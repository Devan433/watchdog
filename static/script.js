document.addEventListener('DOMContentLoaded', () => {
    ...
    scanForm.addEventListener('submit', async (e) => {
        ...
        try {
            // Run UI simulation and actual fetch in parallel
            ...
            showError("Failed to connect to the scanning service. Make sure the backend is running.");
            resetUI();
        }
    });
    ...
    async function simulateScanningSteps() {
        for (let i = 0; i < steps.length; i++) {
            ...
            // Wait slightly random time per step to feel real
    ...
    function renderResults(data) {
        ...
        const checkCategories = {
            "HTTPS Enforcement": { label: "HTTPS Enabled" },
            "Content-Security-Policy": { label: "Content Security Policy" },
            "Cookie: session — HttpOnly": { label: "Secure Cookies" },
            "Secret Leak": { label: "No Leaked Secrets" }
        };

        allFindings.forEach(f => {
            if (checkCategories[f.check_name] || (f.category === "secret_leak" && f.passed)) {
                // Determine label
                let label = f.check_name;
                if (f.category === "secret_leak") label = "No Leaked Secrets";
                if (checkCategories[f.check_name]) label = checkCategories[f.check_name].label;

                // Add to checklist but only unique ones or prioritize failures
                const existing = Array.from(checklistContainer.children).find(el => el.dataset.label === label);
                if (existing) {
                    if (!f.passed && existing.dataset.passed === "true") {
                        existing.outerHTML = renderChecklistItem(label, false);
                    }
                } else {
                    checklistContainer.innerHTML += renderChecklistItem(label, f.passed);
                }
            }
        });

        // Recommendations
        const failedFindings = allFindings.filter(f => !f.passed).sort((a, b) => {
        ...
        if (failedFindings.length === 0) {
            ...
            // Take top 5 unique recommendations
                ...
                findingsContainer.innerHTML += `
                    <div class="finding-card">
                        <div class="finding-header">
                            <span class="severity-badge bg-${f.severity}">${f.severity}</span>
                            <span class="finding-title">${f.check_name}</span>
                        </div>
                        <div class="finding-body">
                            <p><strong>Issue:</strong> ${f.detail}</p>
                            <p><strong>Recommended Fix:</strong> ${f.fix}</p>
        ...
        // Attack Surface Routes (from endpoints check)
        const routesContainer = document.getElementById('routes-container');
        routesContainer.innerHTML = '';
        const endpointFindings = allFindings.filter(f => f.category === 'endpoints' && !f.passed);
        ...
        if (endpointFindings.length === 0) {
            ...
            endpointFindings.forEach(f => {
                ...
                routesContainer.innerHTML += `<span class="route-chip ${isRisky ? 'risky' : ''}">${routeMatch}</span>`;
            });
        }
    }
    ...
    function renderChecklistItem(label, passed) {
        ...
        return `<li class="checklist-item" data-label="${label}" data-passed="${passed}">${icon} <span>${label}</span></li>`;
    }
